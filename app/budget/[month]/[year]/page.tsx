...
export default function HandleBudgetPage() {
  ...
  useEffect(() => {
    ...
    totalBudgeted();
  }, [cats, formState]); //set the dependency to cats so that it recalculates whenever somthing changes in the code
  ...
  const [ddlMonth, setDdlMonth] = useState(
    months.filter((v) => v.value === Number(month))[0] // changed parseInt to Number as the month type is string | string[].
  );
  ...
  useEffect(() => {
    ...
    if (formState.message) setSuccess(true);
    setTimeout(() => {
      setSuccess(false);
    }, 3000);
  }, [formState]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [month, year, cats, formState]); //formstate and cats state dependency added
  if (loading) {
    return (
      //implement a loader
      <div
